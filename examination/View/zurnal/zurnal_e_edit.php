			...
			<div class="sticky_body">
				<div class='top_of_page'>
					...
					<div class="page_title">
						...
						<h3><a href="/ARM/basis/personal/edit/<?= $resp_pers_e['id'] ?>?mode=responsible_persons_info" target = "_blank"><span><i class="icon-rp"></i></span>&nbsp <?php echo $resp_pers_e['secondname']." ".$resp_pers_e['firstname']." ".$resp_pers_e['lastname']." (".$resp_pers_e['post'].")"?></a></h3>
					</div>
					<div class ='nav_buttons'> 
						<a href="/ARM/examination/zurnal/zurnal_e/"><button class="button_filter" ><span><i class="icon-list"></i></span>Реестр ОЛЭ</button></a>				
			...
			<table class="main_table">
                <thead>
					<tr>
						...
						<th width="35%">Структурное подразделение/<br>члены комиссии</th>
...
//print_r($test_book_e_history);
					if(count($test_book_e_history) > 0){
					$n = 1; // номер по порядку

					foreach($test_book_e_history as $zurnal_history):
					...
					<tr class="item-<?= $zurnal_history['id'] ?>">
						...
						<td class="list_td"><?= $zurnal_history['el_group'] ?></td>
						<td class="list_td"><?= $zurnal_history['spr_test_reasons_elektro_name'] ?></td>
						...
						<td class="list_td">
							<div class="tooltip">
								<button class="button-operations" onclick="report_zurnal.zurnalMain_e(<?= $zurnal_history['id'] ?>)">
									<i class="icon-docs"></i><span class = "tooltiptext">протокол</span>
								</button>
							</div>							
						</td>
