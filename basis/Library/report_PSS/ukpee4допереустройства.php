...
//запрос -----на адрес потребителя-----------------------------------------------------------------------------------------------------------------------------------------------------
	...
	$adr = mysqli_fetch_array($arr_adr,MYSQLI_ASSOC);  	//передали в массив адрес из 11 значений (от 0 до 10)
	$adr['on_ind']=1;					//добавили в массив значение -- [11] вывод индекса
	$adr['on_ct_z']=1;					//добавили в массив значение -- [12] вывод района города
	//print_r($adr);
...
//запрос ----оновные данные потребителя-----------------------------------------------------------------------------------------------------------------------------------------------------
	...
	spr_kind_of_activity.name AS activity,
	spr_shift_of_work.name AS sh_work,
...
//запрос ----- на объекты потребителя--- есть ли они, запомнил их  в массиве $arr_obj-----------------------------------------------------------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------создаем форму карточки ---------------------------------------------------------------------------------------------------------------
	$rn=3; 																							//кол-во пустых строк в таблицах
	...
	$sheet->mergeCells('G'.$r.':Z'.$r);
	$spreadsheet->getActiveSheet()->getStyle('G'.$r.':Z'.$r) ->getBorders() ->getBottom() ->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN); 
	$sheet->setCellValue('G'.$r, $r_sub['activity']);
	...
	$sheet->mergeCells('G'.$r.':Z'.$r);
	$spreadsheet->getActiveSheet()->getStyle('G'.$r.':Z'.$r) ->getBorders() ->getBottom() ->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN); 
	$sheet->setCellValue('G'.$r, $r_sub['sh_work']);	
	...
	if (mysqli_num_rows($arr_obj)==0){
			...
			while($r_obj = mysqli_fetch_array($arr_obj)){
				//запрос ------ на адрес в объекте
					...
					$adr = mysqli_fetch_array($arr_adr_ob,MYSQLI_ASSOC);			//передали в массив значения
				//-------------------------------------------- 
				
				//обработка запроса для адреса
					$adr['on_ind']=0;								//добавили в массив значение -- [11] вывод индекса
					$adr['on_ct_z']=1;								//добавили в массив значение -- [12] вывод района города
					$address_ob="-";
					$address_ob=fun_address($adr);
				// обработка для надежности электроснабжения и мощности
					$ss=$adr['ec1']+$adr['ec2']+$adr['ec3']; if ($ss==0) {$ss="-";}
					$cat="-";
					if ($adr['ec3']>0) {$cat="III";};
					if ($adr['ec2']>0) {$cat="II";};
					if ($adr['ec1']>0) {$cat="I";};
					if ($adr['ec1p']>0) {$cat="I особая";};
				//-------------------------------------------- 
				...
				$sheet->mergeCells('V'.$r.':Z'.$r);	$sheet->setCellValue('V'.$r, $r_sub['activity']); $sheet->getStyle('V'.$r) ->getFont() ->setSize(8);
				
				//запрос ------ на субабоненты в объекте +адрес и название потребителя
					$arr_subobj_ob=mysqli_query($link,"
					SELECT 
						reestr_object.name,
						reestr_object.address_index AS ind,	
						spr_region.name AS rgn, 	  
					...
					FROM `reestr_object`
						...
						Left JOIN spr_type_city ON reestr_object.address_city_type=spr_type_city.id
						Left JOIN spr_type_street ON reestr_object.address_street_type=spr_type_street.id	
					WHERE reestr_object.e_subobj_obj=".$r_obj['id']);
					$so = mysqli_fetch_array($arr_subobj_ob,MYSQLI_ASSOC);			//передали в массив значения
					//print_r($so);
				//-------------------------------------------- 	
			}
	}
	...
	for($tr=1; $tr<=$rn; $tr++) {								//таблица строки
	$r++;																			
	$sheet->getRowDimension($r)->setRowHeight(35);
	$spreadsheet->getActiveSheet()->getStyle('A'.$r.':Z'.$r) ->getBorders() ->getallBorders() ->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
	$sheet->mergeCells('A'.$r.':A'.$r);	$sheet->setCellValue('A'.$r, '1'); 
	$sheet->mergeCells('B'.$r.':G'.$r);	$sheet->setCellValue('B'.$r, $so['name']); 
