...
//Установка ширины столбца
	...
	//$trc = (mysqli_num_rows($arr_obj)<=3) ? 3 : mysqli_num_rows($arr_obj); //короткий if
	if (mysqli_num_rows($arr_obj)==0){
			$r++;																			
			$sheet->getRowDimension($r)->setRowHeight(35);
			$spreadsheet->getActiveSheet()->getStyle('A'.$r.':Z'.$r) ->getBorders() ->getallBorders() ->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
			$sheet->mergeCells('A'.$r.':G'.$r);	$sheet->mergeCells('H'.$r.':N'.$r);	$sheet->mergeCells('O'.$r.':P'.$r); $sheet->mergeCells('Q'.$r.':R'.$r);	$sheet->mergeCells('S'.$r.':U'.$r);	$sheet->mergeCells('V'.$r.':Z'.$r);	
		}else{
			while($r_obj = mysqli_fetch_array($arr_obj)){
				...
				$sheet->mergeCells('H'.$r.':N'.$r);	$sheet->setCellValue('H'.$r, '-');
