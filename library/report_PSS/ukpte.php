...
//Установка ширины столбца
	...
	$sheet->setCellValue('A'.$r, 'потребителя тепловой энергии');
	...
	$sheet->getCell('B'.$r)->getStyle() ->getAlignment() ->setHorizontal('left');
	$sheet->setCellValue('B'.$r, 'Источник теплоснабжения:');
	...
	$r++; $r++;
	$sheet->mergeCells('A'.$r.':Z'.$r);
	$sheet->setCellValue('A'.$r, 'Краткая характеристика теплоснабжения потребителя тепловой энергии');
	...
	$sheet->mergeCells('A'.$r.':A'.$r);		$sheet->setCellValue('A'.$r, '№ п/п');
	$sheet->mergeCells('B'.$r.':l'.$r);		$sheet->setCellValue('b'.$r, 'Перечень объектов');
	$sheet->mergeCells('m'.$r.':R'.$r);		$sheet->setCellValue('m'.$r, 'Категория надежности теплоснабжения объекта (систем теплопотребления объекта)'); $sheet->getStyle('m'.$r) ->getFont() ->setSize(10);
	$sheet->mergeCells('S'.$r.':Z'.$r);		$sheet->setCellValue('s'.$r, 'Место нахождения');
	...
	$sheet->mergeCells('b'.$r.':l'.$r);	$sheet->setCellValue('b'.$r, $n++); $sheet->getStyle('b'.$r) ->getFont() ->setSize(8);
	$sheet->mergeCells('m'.$r.':R'.$r);	$sheet->setCellValue('m'.$r, $n++); $sheet->getStyle('m'.$r) ->getFont() ->setSize(8);
	...
	$sheet->mergeCells('b'.$r.':l'.$r);	$sheet->setCellValue('b'.$r, 'nhek asdf asdfsa'); 
	$sheet->mergeCells('m'.$r.':R'.$r);	$sheet->setCellValue('m'.$r, 'IV');
	$sheet->mergeCells('s'.$r.':Z'.$r);	$sheet->setCellValue('s'.$r, 'адрес');
	}
	//--------------------------------------------------------------------------------------------------------
	...
	$sheet->getCell('A'.$r)->getStyle() ->getAlignment() ->setHorizontal('left');
	$sheet->setCellValue('A'.$r, 'Максимальная часовая нагрузка:');
	...
	$sheet->setCellValue('s'.$r, 'отопление');
	$sheet->mergeCells('v'.$r.':x'.$r);
	$spreadsheet->getActiveSheet()->getStyle('v'.$r.':x'.$r) ->getBorders() ->getallBorders() ->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
	$sheet->setCellValue('v'.$r, '5466');
	$sheet->mergeCells('y'.$r.':z'.$r);
	$sheet->getCell('y'.$r)->getStyle() ->getAlignment() ->setHorizontal('left');
	$sheet->setCellValue('y'.$r, ' Гкал/ч;');
	
	$r++;
	$sheet->mergeCells('A'.$r.':h'.$r);
	$sheet->getCell('A'.$r)->getStyle() ->getAlignment() ->setHorizontal('left');
	$sheet->setCellValue('A'.$r, 'горячее водоснабжение:');
	$sheet->mergeCells('I'.$r.':k'.$r);
	$spreadsheet->getActiveSheet()->getStyle('i'.$r.':k'.$r) ->getBorders() ->getallBorders() ->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
	$sheet->setCellValue('i'.$r, '8888');
	$sheet->mergeCells('l'.$r.':m'.$r);
	$sheet->getCell('l'.$r)->getStyle() ->getAlignment() ->setHorizontal('left');
	$sheet->setCellValue('l'.$r, ' Гкал/ч;');
	$sheet->mergeCells('n'.$r.':p'.$r);
	$sheet->setCellValue('n'.$r, 'технология');
	$sheet->mergeCells('q'.$r.':s'.$r);
	$spreadsheet->getActiveSheet()->getStyle('q'.$r.':s'.$r) ->getBorders() ->getallBorders() ->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
	$sheet->setCellValue('q'.$r, '80800');
	$sheet->mergeCells('t'.$r.':u'.$r);
	$sheet->getCell('t'.$r)->getStyle() ->getAlignment() ->setHorizontal('left');
	$sheet->setCellValue('t'.$r, ' Гкал/ч;');	
	$sheet->mergeCells('v'.$r.':x'.$r);
	$spreadsheet->getActiveSheet()->getStyle('v'.$r.':x'.$r) ->getBorders() ->getallBorders() ->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
	$sheet->setCellValue('v'.$r, '02326');
	$sheet->mergeCells('y'.$r.':z'.$r);
	$sheet->getCell('y'.$r)->getStyle() ->getAlignment() ->setHorizontal('left');
	$sheet->setCellValue('y'.$r, ' т/ч;');

		$r++;
	$sheet->mergeCells('A'.$r.':d'.$r);
	$sheet->getCell('A'.$r)->getStyle() ->getAlignment() ->setHorizontal('left');
	$sheet->setCellValue('A'.$r, 'вентиляция:');
	$sheet->mergeCells('e'.$r.':g'.$r);
	$spreadsheet->getActiveSheet()->getStyle('e'.$r.':g'.$r) ->getBorders() ->getallBorders() ->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
	$sheet->setCellValue('e'.$r, '345886');
	$sheet->mergeCells('h'.$r.':i'.$r);
	$sheet->getCell('h'.$r)->getStyle() ->getAlignment() ->setHorizontal('left');
	$sheet->setCellValue('h'.$r, ' Гкал/ч.');
	
	
	$r++;	$r++;
	$sheet->mergeCells('A'.$r.':z'.$r);
	$sheet->getCell('a'.$r)->getStyle() ->getAlignment() ->setHorizontal('left');
	$sheet->setCellValue('a'.$r, 'Субабоненты');
	
		$r++;
	$sheet->mergeCells('a'.$r.':z'.$r);
	$spreadsheet->getActiveSheet()->getStyle('a'.$r.':z'.$r) ->getBorders() ->getBottom() ->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN); //толнкая линия границы
	$sheet->setCellValue('a'.$r, 'отсутствуют');
	...
	$sheet->getCell('A'.$r)->getStyle() ->getAlignment() ->setHorizontal('left');
	$sheet->setCellValue('A'.$r, 'Актом от');
	$sheet->mergeCells('D'.$r.':j'.$r);
	$spreadsheet->getActiveSheet()->getStyle('D'.$r.':j'.$r) ->getBorders() ->getBottom() ->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN); 
	$sheet->setCellValue('d'.$r, $temp_date.'г.');
	$sheet->mergeCells('k'.$r.':p'.$r);
	$sheet->setCellValue('k'.$r, 'аварийная бронь');
	$sheet->mergeCells('q'.$r.':s'.$r);
	$spreadsheet->getActiveSheet()->getStyle('q'.$r.':s'.$r) ->getBorders() ->getallBorders() ->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
	$sheet->setCellValue('q'.$r, '8800');
	$sheet->mergeCells('t'.$r.':u'.$r);
	$sheet->getCell('t'.$r)->getStyle() ->getAlignment() ->setHorizontal('left');
	$sheet->setCellValue('t'.$r, ' Гкал/ч;');	
	$sheet->mergeCells('v'.$r.':x'.$r);
	$spreadsheet->getActiveSheet()->getStyle('v'.$r.':x'.$r) ->getBorders() ->getallBorders() ->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
	$sheet->setCellValue('v'.$r, '2326');
	$sheet->mergeCells('y'.$r.':z'.$r);
	$sheet->getCell('y'.$r)->getStyle() ->getAlignment() ->setHorizontal('left');
	$sheet->setCellValue('y'.$r, ' т/ч.');
	...
	$sheet->mergeCells('A'.$r.':f'.$r);
	$sheet->getCell('A'.$r)->getStyle() ->getAlignment() ->setHorizontal('left');
	$sheet->setCellValue('A'.$r, 'Технологическая бронь');
	$sheet->mergeCells('g'.$r.':i'.$r);
	$spreadsheet->getActiveSheet()->getStyle('g'.$r.':i'.$r) ->getBorders() ->getallBorders() ->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
	$sheet->setCellValue('g'.$r, '5466');
	$sheet->mergeCells('j'.$r.':k'.$r);
	$sheet->getCell('j'.$r)->getStyle() ->getAlignment() ->setHorizontal('left');
	$sheet->setCellValue('j'.$r, ' Гкал/ч;');
	$sheet->mergeCells('l'.$r.':n'.$r);
	$spreadsheet->getActiveSheet()->getStyle('l'.$r.':n'.$r) ->getBorders() ->getallBorders() ->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
	$sheet->setCellValue('l'.$r, '6345866');
	$sheet->mergeCells('o'.$r.':p'.$r);
	$sheet->getCell('o'.$r)->getStyle() ->getAlignment() ->setHorizontal('left');
	$sheet->setCellValue('o'.$r, ' т/ч;');
	$sheet->mergeCells('q'.$r.':u'.$r);
	$sheet->setCellValue('q'.$r, 'продолжительностью');
	$sheet->mergeCells('v'.$r.':x'.$r);
	$spreadsheet->getActiveSheet()->getStyle('v'.$r.':x'.$r) ->getBorders() ->getallBorders() ->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
	$sheet->setCellValue('v'.$r, '46');
	$sheet->mergeCells('y'.$r.':z'.$r);
	$sheet->getCell('y'.$r)->getStyle() ->getAlignment() ->setHorizontal('left');
	$sheet->setCellValue('y'.$r, ' ч.;');
	
		$r++;$r++;
	$sheet->mergeCells('A'.$r.':Z'.$r);
	$sheet->getCell('A'.$r)->getStyle() ->getAlignment() ->setHorizontal('left');
	$sheet->setCellValue('A'.$r, 'Основное теплоипользующее оборудование');
	//таблица ------------------------------------------------------------------------------------------------------------------
	$r++;													//таблица шапка				
	...
	$sheet->mergeCells('i'.$r.':m'.$r);		$sheet->setCellValue('i'.$r, 'Назначение');
	$sheet->mergeCells('n'.$r.':p'.$r);		$sheet->setCellValue('n'.$r, 'Количество, шт.');							
	$sheet->mergeCells('q'.$r.':V'.$r);		$sheet->setCellValue('q'.$r, 'Место установки');										
	$sheet->mergeCells('W'.$r.':Z'.$r);		$sheet->setCellValue('W'.$r, 'Тип'); 
