$(document).ready(function(){
	...
	/*$('#checkUNPinBASIS').click(function(){
		...
		if($("#formUNP").val().length == 9){
			...
			mainSearch();
			$('#checkUNPinBASIS').prop('disabled', true);
		}
		
	});*/
	
	$('input[name="unp"][id="formUNP"]').bind('input', function(){
		var searchString    = $("#formUNP").val();
	...
	$('#formUNP').bind('input',function(){
		...
		if($("#formUNP").val().length < 9){
			...
			$('#checkUNPinBASIS').prop('disabled', false);	
		}
	});
	...
	$("button#portal").click(function(){
		event.preventDefault();
		checkUnpPortal();
		$('#portal').prop('disabled', true);
		$('#new_name_unp').prop('disabled', false);
		$('.unp_right_part').css({'display':'initial'});
	});
	
	$("input#formUNP").click(function(){
		$("#formName, #formShortName, #formIndex, #formRegion, #formDistrict, #spr_type_city, #formCity, #formCityZone, #spr_type_street, #formStreet, #formBuilding, #formFlat, #formLiquidated, #portal").prop('disabled', true);
		//$("#formName, #formShortName, #formIndex, #formRegion, #formDistrict, #spr_type_city, #formCity, #formCityZone, #spr_type_street, #formStreet, #formBuilding, #formFlat, #formLiquidated, #portal").val('').change();
		$('#formUNP').val('').change();
		$('#checkUNPinBASIS').prop('disabled', true);
		$('.unp_right_part').css({'display':'none'});
		$("#search_result_unp").empty();
		$("#portal_message").empty();
	});
	...
	$("button#half_fill_form").click(function(){
		event.preventDefault();
		if($("#portal_sost").text() === "Действующий" || $("#portal_sost").text() === "В состоянии ликвидации"){
            ...
            $.ajax({
				...
				//	console.log(html);
					...
					if(html.length > 0){
						...
						$("#formPage select").prop('disabled', false);
						$("#formPage button[class='btn_add_fields']").prop('disabled', false);
						/*$("#formPage #formLiquidated").prop('disabled', false);
						$("#formPage #formRegion").prop('disabled', false);
						$("#formPage #spr_type_street").prop('disabled', false);
						$("#formPage button[class='btn_add_fields']").prop('disabled', true);
						*/
			...
			$.ajax({
				...
				success: function(result){
					...
					if(result.length > 3){
						...
						$.each(data, function(ind, val){
							switch(ind){
								...
								case 'name':
									...
									$('#nm_full_name').text(val);
								break
								case 'name_short':
									$('#portal_short_name').html(val);
									$('#nm_short_name').text(val);
								break
								case 'address_portal':
						...
						$('#nm_full_name').attr('disabled', false);
						$('#nm_short_name').attr('disabled', false);
						
						$('.unp_right_part p').css({'display':'none'});
						$('.unp_right_part button').css({'display':'none'});
						$('#portal_message').css({'display':'block'});
						$('#portal_message').html('Автоматический поиск не нашёл информацию по данному УНП.<br/>Проверьте вручную.');
					}
