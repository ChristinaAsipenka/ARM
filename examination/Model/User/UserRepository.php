...
/********** в одну функцию  ****************/
 ...
 public function getUsersForCommission($fio)
    {
        $sql = $this->queryBuilder->select('users.id,  users.fio, users.login, users.name, users.fam, users.otch, users.dolgnost,  users.spr_cod_otd, users.spr_cod_podrazd, users.podrazdelenie, spr_branch.sokr_name as spr_branch_sokr_name, spr_otdel.sokr_name as spr_otdel_sokr_name, spr_podrazdelenie.sokr_name as spr_podrazdelenie_sokr_name')
            ->from('users')
			->joinLeftTable('spr_branch', 'users.spr_cod_branch = spr_branch.id')
			->joinLeftTable('spr_otdel', 'users.spr_cod_otd = spr_otdel.id')
			->joinLeftTable('spr_podrazdelenie', 'users.spr_cod_podrazd = spr_podrazdelenie.id')
			//->where('rules.arm_gruppa', 3)
			->where('fio', '%'.$fio.'%', 'LIKE')
            ->orderBy('fio', 'ASC')
            ->sql();
