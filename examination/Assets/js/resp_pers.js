...
var resp_pers = {
    ...
    add: function() {
		...
		error = resp_pers.checkFields();

		if(error == 0){		
		  $.ajax({
				url: '/ARM/examination/responsible_persons/add/',
				...
				success: function(result){
					...
					if(window.location.href.indexOf('examination') != -1){
					
						$('#id_rp').val(result);
						
					$('button.test_chose_unp').css('display','none');	
					$('button.test_create_rp').text('Сохранить');	
					$('input[name=resp_pers_secondname]').prop('disabled',true);
					$('input[name=resp_pers_firstname]').prop('disabled',true);
					$('input[name=resp_pers_lastname]').prop('disabled',true);
					$('input[name=resp_pers_post]').prop('disabled',true);
					$('input[name=resp_pers_post_data]').prop('disabled',true);
    ...
    update: function() {
		...
		if(error == 0){		
			...
			$.ajax({
				url: '/ARM/examination/responsible_persons/update/',
				type: this.ajaxMethod,
				data: formData,
				cache: false,
				processData: false,
	...
	changePost : function(){
        ...
        var formData = new FormData();
		formData.append('resp_pers_id', $('#resp_pers_id').val());
		conf = confirm('После смены должности последняя проверка знаний будет помечена как недействительная. Продолжить?');
			
		if(conf==false){
			return false;
		}else{
			$('input#resp_pers_post').prop('disabled', false);
			$('input#resp_pers_post_data').prop('disabled', false);
		}
		
		
			$.ajax({
				url: '/ARM/examination/responsible_persons/update_post_e/',
				type: this.ajaxMethod,
				data: formData,
				cache: false,
				processData: false,
			...
			$.ajax({
				url: '/ARM/examination/responsible_persons/update_post_t/',
				type: this.ajaxMethod,
				data: formData,
				cache: false,
				processData: false,
			...
			$.ajax({
				url: '/ARM/examination/responsible_persons/update_post_g/',
				type: this.ajaxMethod,
				data: formData,
				cache: false,
				processData: false,
	...
	checkFields: function(){
		if($("#resp_pers_id").val() > 0){
			var fields = [];
		}else{
			var fields = ["resp_pers_secondname", "resp_pers_firstname", "resp_pers_lastname", "resp_pers_post", "resp_pers_post_data", "formUnpId"];
		}
	...
	remove: function(itemId) {
	    ...
	    if (itemId < 1) {
			...
			$.ajax({
				url: '/ARM/examination/responsible_persons/removeItem/',
				type: this.ajaxMethod,
				data: formData,
				cache: false,
				processData: false,
	...
	RespPersInfo: function(resppersId){
		...
		$.ajax({
				url: '/ARM/examination/responsible_persons/info/'+resppersId,
				type: this.ajaxMethod,
				data: formData,
				cache: false,
				processData: false,
	...
	search_hide_show: function(value) {
		if(value == 1){
			...
			var formData = new FormData();
				...
				$.ajax({
					url: '/ARM/examination/responsible_persons/ListRespPers/',
					type: this.ajaxMethod,
					data: formData,
					cache: false,
					processData: false,
...
$(window).ready(function(){
	...
	if(window.location.href.indexOf("responsible_persons/create") != -1){
