        ...
        <div class="container">
			<div class="sticky_body">
				<div class='top_of_page'>
					...
					<div class="page_title">
						...
						<h3><a href="/ARM/basis/personal/edit/<?= $resp_pers_g['id'] ?>?mode=responsible_persons_info" target = "_blank"><span><i class="icon-rp"></i></span>&nbsp <?php echo $resp_pers_g['secondname']." ".$resp_pers_g['firstname']." ".$resp_pers_g['lastname']." (".$resp_pers_g['post'].")"?></a></h3>
					</div>
					<div class ='nav_buttons'> 
						<a href="/ARM/examination/zurnal/zurnal_g/"><button class="button_filter" ><span><i class="icon-list"></i></span>Реестр ОЛТ</button></a>				
			...
			<table class="main_table">
				...
				<tbody>
					...
					if(count($test_book_g) > 0){
					$n = 1; // номер по порядку

					foreach($test_book_g as $zurnal):
					...
					<!--tr class="item-<?//= $zurnal['id'] ?>"-->
						...
						<td class="list_td"><?= $zurnal['spr_test_reasons_gaz_name'] ?></td>	
						...
						<td class="list_td">
							<div class="tooltip">
								<button class="button-operations" onclick="report_zurnal.zurnalMain_g(<?= $zurnal['id'] ?>)">
...
//print_r($test_book_e_history);
					if(count($test_book_g_history) > 0){
					$n = 1; // номер по порядку

					foreach($test_book_g_history as $zurnal_history):
					...
					<tr class="item-<?= $zurnal_history['id'] ?>">
						...
						<td class="list_td"><?= $zurnal_history['spr_test_reasons_gaz_name'] ?></td>
						...
						<td class="list_td">
							<div class="tooltip">
								<button class="button-operations" onclick="report_zurnal.zurnalMain_g(<?= $zurnal_history['id'] ?>)">
									<i class="icon-docs"></i><span class = "tooltiptext">протокол</span>
								</button>
							</div>							
						</td>
